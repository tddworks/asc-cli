...
### Fixed
- **Empty `imageAsset` no longer leaks to clients while ASC is still processing the upload** — right after a review screenshot or promotional image upload commits, ASC returns the asset record with `imageAsset: {templateURL: "", width: 0, height: 0}` because server-side processing hasn't finished. The previous mapper accepted those zero values, so REST clients saw a non-nil but unusable `imageAsset`. Both `SDKInAppPurchaseReviewRepository` and `SDKSubscriptionReviewRepository` now require a non-empty `templateURL` and positive dimensions; otherwise the field is `nil` and `encodeIfPresent` omits it from JSON entirely. Frontends (e.g. AppNexus's review tabs) can show a local preview while `imageAsset` is absent and poll `GET /api/v1/iap/:iapId/images` (or `…/review-screenshot`) until the field appears — same pattern the iOS app uses with its local-fileURL fallback in `IAPReviewTabContent`.

...
### Changed
- **`InAppPurchase.submit` affordance auto-dispatches to iris vs. public SDK** — when an IAP listing has no sibling in an approved-or-past state (`approved | removedFromSale | developerRemovedFromSale`), every unapproved IAP gets `isFirstTimeSubmission: true` and its `submit` affordance resolves to `asc iris iap-submissions create --iap-id <id>` (the path Apple actually accepts for first-time submissions). Once one IAP ships, every subsequent listing flips them all back and `submit` resolves to the existing `asc iap submit`. Detection is per-batch in `SDKInAppPurchaseRepository.listInAppPurchases` — zero extra API calls. `asc iap get` lacks batch context and always resolves to the SDK path; documented as a known inconsistency. The agent reads one `submit` affordance and runs it — no need to know iris-vs-sdk exists. New `InAppPurchaseState.hasBeenApproved` semantic boolean. New `iris iap-submissions` REST route registered with the path resolver so `submit` `_links` resolve to `/api/v1/iris/iap/<id>/submissions` for first-time IAPs.
